...
class UAIParser:
    ...
    def parse(self) -> None:
        ...
        with open(self.filepath, 'r') as file:
            ...
            for _ in range(num_factors):
                ...
                index += 1
                parent_list = list(map(int, info[index:index + num_parents]))
                self.parents.append(parent_list)
                index += num_parents + 1
    ...
    def calculate_probability(self, outcome: list[int]) -> float:
        ...
        for i, value in enumerate(outcome):
            if len(self.parents[i]) == 0:
                ...
                parent_values = tuple(
                    outcome[parent] for parent in self.parents[i]
                )
    ...
    def generate_data(self) -> DataFrame:
        ...
        probabilities = [
            [sublist, val] for sublist, val in zip(outcomes, probs)
        ]
